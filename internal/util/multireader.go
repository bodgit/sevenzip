...
/*
https://github.com/go4org/go4/blob/master/readerutil/multireaderat.go

- Remove SizeReaderAt and replaced with io.SectionReader
*/
...
func NewMultiReaderAt(parts ...*io.SectionReader) *MuliReaderAt {
	m := &MuliReaderAt{
		parts: make([]offsetAndSource, 0, len(parts)),
	}
	var off int64
	for _, p := range parts {
		m.parts = append(m.parts, offsetAndSource{p, off})
		off += p.Size()
	}
	m.size = off
	return m
...
type MuliReaderAt struct {
	parts []offsetAndSource
	size  int64
}

func (m *MuliReaderAt) Size() int64 { return m.size }

func (m *MuliReaderAt) ReadAt(p []byte, off int64) (n int, err error) {
	wantN := len(p)
